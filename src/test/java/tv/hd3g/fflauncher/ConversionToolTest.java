...
public class ConversionToolTest extends TestCase {
	...
	public void test() throws IOException {
		...
		final LinkedBlockingQueue<ExecProcessText> ept_catched_list = new LinkedBlockingQueue<>();
		final Consumer<ExecProcessText> new_instance_catcher = ept -> ept_catched_list.add(ept);
		ct.setExecProcessCatcher(new_instance_catcher);

		final File working_directory = ct.getExecutable().getParentFile();
		...
		final DeprecatedCommandLine command_line = ct.getCommandLine();
	...
	public void testCatchMissingOutVar() throws IOException {
		...
		class CT extends ConversionTool {
			...
			public CT() throws FileNotFoundException {
				super(new ExecutableFinder(), new DeprecatedCommandLineProcessor().createCommandLine("java"));
			}
