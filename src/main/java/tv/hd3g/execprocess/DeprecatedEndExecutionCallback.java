...
@Deprecated
class DeprecatedEndExecutionCallback<T extends ExecProcessResult> {

	private final Consumer<T> onEnd;
	protected final Executor executor;

	DeprecatedEndExecutionCallback(final Consumer<T> onEnd, final Executor executor) {
		this.onEnd = onEnd;
		if (onEnd == null) {
	...
	void onEnd(final ExecProcessResult source) {
		executor.execute(() -> {
